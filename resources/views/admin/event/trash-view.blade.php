        ...
        <div class="container">
            ...
            @foreach ($events as $event)
                <tr>
                    ...
                    <td>{{ $event->point3 }}</td>
                    <td>{{ $event->end }}</td>
                    <td style="text-align: center;"> <img src="{{ $event->getFirstMediaUrl('event-collection', 'thumb') }}" > </td>
                    <td>{{ $event->updated_at->diffForHumans() }}</td>
                    <td>
                        {!! Form::open(['route' => ['event.trashRestore', $event->id], 'method' => 'post']) !!}
                        {{-- <a href="{{ route('phone.show', $phone->id) }}" title="show" class="crud-spacing">
                            <i class="fas fa-eye text-success  fa-lg"></i>
                        </a> --}}
                        {{-- <a href="{{ route('event.edit', $event->id) }}" class="crud-spacing">
                    ...
                    {!! Form::button('<i class="text-success"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-folder-symlink-fill" viewBox="0 0 16 16">
                          ...
                          </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to delete event '. $event->heading.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Restore File', 'class' => 'crud_spacing']) !!}
                    {!! Form::close() !!}

                    {!! Form::open(['route' => ['event.trashDestroy', $event->id], 'method' => 'post']) !!}
                            ...
                            </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to delete: '. $event->heading.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Permanently Delete File', 'class' => 'crud_spacing']) !!}
                    {!! Form::close() !!}
                    </td>
                </tr>
            @endforeach
            ...
            {!! $events->appends(\Request::except('page'))->render('pagination::bootstrap-4') !!}
        </div>
    @endsection
