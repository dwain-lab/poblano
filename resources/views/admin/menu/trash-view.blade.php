        ...
        <div class="container">
            ...
            @foreach ($menus as $menu)
                <tr>
                    ...
                    <td style="text-align: center;"> <img src="{{ $menu->getFirstMediaUrl('menus-collection', 'thumb') }}" > </td>
                    <td>{{ $menu->updated_at->diffForHumans() }}</td>
                    <td>
                        {!! Form::open(['route' => ['menu.trashRestore', $menu->id], 'method' => 'post']) !!}
                        {{-- <a href="{{ route('phone.show', $phone->id) }}" title="show" class="crud-spacing">
                            <i class="fas fa-eye text-success  fa-lg"></i>
                        </a> --}}
                        {{-- <a href="{{ route('menu.edit', $menu->id) }}" class="crud-spacing">
                    ...
                    {!! Form::button('<i class="text-success"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-folder-symlink-fill" viewBox="0 0 16 16">
                          ...
                          </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to restore '. $menu->name.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Restore File', 'class' => 'crud_spacing']) !!}
                    {!! Form::close() !!}

                    {!! Form::open(['route' => ['menu.trashDestroy', $menu->id], 'method' => 'post']) !!}
                            ...
                            </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to permanently delete: '. $menu->name.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Permanently Delete File', 'class' => 'crud_spacing']) !!}
                    {!! Form::close() !!}
                    </td>
                </tr>
            @endforeach
