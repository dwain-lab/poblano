        ...
        <div class="container">
            ...
            @foreach ($menu_categories as $menu_category)
                <tr>
                    ...
                    <td >
                            {!! Form::open(['route' => ['menu_category.trashRestore', $menu_category->id], 'method' => 'post']) !!}
                                {{-- <a href="{{ route('phone.show', $phone->id) }}" title="show" class="crud-spacing">
                                    <i class="fas fa-eye text-success  fa-lg"></i>
                                </a> --}}
                                {{-- <a href="{{ route('menu_category.edit', $menu_category->id) }}" class="crud-spacing">
                            ...
                            {!! Form::button('<i class="text-success"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-folder-symlink-fill" viewBox="0 0 16 16">
                                  ...
                                  </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to restore: '. $menu_category->name.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Restore File', 'class' => 'crud_spacing']) !!}
                            {!! Form::close() !!}

                            {!! Form::open(['route' => ['menu_category.trashDestroy', $menu_category->id], 'method' => 'post']) !!}
                                    ...
                                    </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to delete: '. $menu_category->name.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Permanently Delete File', 'class' => 'crud_spacing']) !!}
                            {!! Form::close() !!}
                            </div>
                    </td>
                </tr>
            ...
            {!! $menu_categories->appends(\Request::except('page'))->render('pagination::bootstrap-4') !!}
        </div>
    @endsection
