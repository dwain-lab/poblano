...
{{--
        <div class="container position-relative text-lg-left aos-init aos-animate">
            ...
            <div style="padding-top:3%">
                ...
                <a class="btn btn-primary" href="{{ route('special.create') }}" title="Add New" style="background-color: rgb(136, 101, 42); hover:opacity:30%"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-plus-circle-fill" viewBox="0 0 16 16">
        ...
        <div class="container">
            ...
            <tr>
                <th> @sortablelink('link' , 'Dish Name') </th>
            ...
            @foreach ($specials as $special)
                <tr>
                    ...
                    <td> {{ $special->intro }} </td>
                    <td> {{ $special->end }} </td>
                    <td style="text-align: center;"> <img src="{{ $special->getFirstMediaUrl('specials-collection', 'thumb') }}" > </td>
                    <td>{{ $special->updated_at->diffForHumans() }}</td>
                    <td>
                        {!! Form::open(['route' => ['special.trashRestore', $special->id], 'method' => 'post']) !!}
                        {{-- <a href="{{ route('phone.show', $phone->id) }}" title="show" class="crud-spacing">
                            <i class="fas fa-eye text-success  fa-lg"></i>
                        </a> --}}
                        {{-- <a href="{{ route('special.edit', $special->id) }}" class="crud-spacing">
                    ...
                    {!! Form::button('<i class="text-success"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-folder-symlink-fill" viewBox="0 0 16 16">
                          ...
                          </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to restore '. $special->link.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Restore File', 'class' => 'crud_spacing']) !!}
                    {!! Form::close() !!}

                    {!! Form::open(['route' => ['special.trashDestroy', $special->id], 'method' => 'post']) !!}
                            ...
                            </svg></i>', ['type' => 'submit', 'onclick' => 'return confirm(\'Are you sure you want to permanently delete: '. $special->link.'?\')', 'style' => 'border: none; background-color:transparent;', 'title'=>'Permanently Delete File', 'class' => 'crud_spacing']) !!}
                    {!! Form::close() !!}
                    </td>
                </tr>
            @endforeach
            ...
            {!! $specials->appends(\Request::except('page'))->render('pagination::bootstrap-4') !!}
        </div>
    @endsection
